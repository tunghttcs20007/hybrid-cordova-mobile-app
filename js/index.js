...
$(document).on('click', '#page-1 #panel-open', function () {
...
$(document).on('click', '#page-setting #btn-backup', function () {
...
$(document).on('click', '#page-setting #btn-reset', function () {
...
function confirmTrip(e) {
	...
	$('#page-create #frm-confirm #AmountOfTheExpense').text(
		$('#page-create #frm-register #AmountOfTheExpense').val()
	);
	$('#page-create #frm-confirm #AdditionalComments').text(
		$('#page-create #frm-register #AdditionalComments').val()
	);
...
function registerTrip(e) {
	...
	let AmountOfTheExpense = $('#page-create #frm-register #AmountOfTheExpense').val();
	let AdditionalComments = $('#page-create #frm-register #rAdditionalCommentsisk').val();

	db.transaction(function (tx) {
		let query = `INSERT INTO Trip (Name, Destination, Description, Risk, Date, AmountOfTheExpense, AdditionalComments) VALUES (?, ?, ?, ?, julianday('${date}'),?,?)`;
		tx.executeSql(
			query,
			[name, destination, description, risk, AmountOfTheExpense, AdditionalComments],
			transactionSuccess,
			transactionError
		);
	...
	db.transaction(function (tx) {
		...
		function transactionSuccess(tx, result) {
			if (result.rows[0] != null) {
				...
				$('#page-detail #detail #AmountOfTheExpense').text(result.rows[0].AmountOfTheExpense);
				$('#page-detail #detail #AdditionalComments').text(result.rows[0].AdditionalComments);
                    ...
                    <div class='list'>
                        <table style='white-space: nowrap; width: 100%;'>
                            <tr style='height: 25px;'>
                                <td>${expense.Type}: ${expense.Amount.toLocaleString(
					'en-US'
				)} VNĐ</td>
